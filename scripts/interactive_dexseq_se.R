...
files <- list.files("data/salmon", "quant.sf", full.names=TRUE, recursive=TRUE) %>%
    { .[str_detect(.,"se/(ery_d|lsk_tip60_wt_[12])")] } %>%
...
saveRDS(dxd, "data/dexseq/dxd_hsc_eryd_se.Rds")

write_tsv(dxr.g, "data/dexseq/dxr_hsc_eryd_se_gene.tsv.gz")
write_tsv(dxr.t, "data/dexseq/dxr_hsc_eryd_se_tx.tsv.gz")
write_tsv(dxr.full, "data/dexseq/dxr_hsc_eryd_se_full.tsv.gz")
...
write_tsv(dex.padj, "data/dexseq/staged_qvals_hsc_eryd_se_fdr5.tsv.gz")
