...
dds <- DESeqDataSetFromMatrix(countData=cts_gene,
                              colData=df_samples,
                              design=~library_type + strand + cell_type)

dds <- DESeq(dds)

dres <- DESeq2::results(dds, contrast=c("cell_type", "EryD", "HSC"))

saveRDS(dds, "data/deseq/dds_hsc_eryd_select.Rds")

dres %>% as_tibble(rownames="Gene")  %>% write_tsv("data/deseq/dres_hsc_eryd_select_gene.tsv.gz")
