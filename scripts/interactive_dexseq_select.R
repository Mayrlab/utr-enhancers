...
files <- list.files("data/salmon", "quant.sf", full.names=TRUE, recursive=TRUE) %>%
    { .[str_detect(.,"/(se/ery_d_[123]|pe/ery_d_gata1_wt|pe/lsk_tip60_wt|pe/lsk_sf3b1_wt_srsf2_wt)")] } %>%
...
saveRDS(dxd, "data/dexseq/dxd_hsc_eryd_select.Rds")
write_tsv(dxr.g, "data/dexseq/dxr_hsc_eryd_select_gene.tsv.gz")
write_tsv(dxr.t, "data/dexseq/dxr_hsc_eryd_select_tx.tsv.gz")
write_tsv(dxr.full, "data/dexseq/dxr_hsc_eryd_select_full.tsv.gz")
library(stageR)
...
write_tsv(dex.padj, "data/dexseq/staged_qvals_hsc_eryd_select_cdr5.tsv.gz")
