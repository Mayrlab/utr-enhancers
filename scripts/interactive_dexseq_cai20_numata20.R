...
files <- list.files("data/salmon", "quant.sf", full.names=TRUE, recursive=TRUE) %>%
    { .[str_detect(.,"/(se/ery_d_[0-9]+|pe/lsk_tip60_wt)")] } %>%
...
saveRDS(dxd, "data/dexseq/dxd_hsc_eryd_cai20_numata20.Rds")
write_tsv(dxr.g, "data/dexseq/dxr_hsc_eryd_cai20_numata20_gene.tsv.gz")
write_tsv(dxr.t, "data/dexseq/dxr_hsc_eryd_cai20_numata20_tx.tsv.gz")
write_tsv(dxr.full, "data/dexseq/dxr_hsc_eryd_cai20_numata20_full.tsv.gz")
library(stageR)
...
write_tsv(dex.padj, "data/dexseq/staged_qvals_hsc_eryd_cai20_numata20_cdr5.tsv.gz")
