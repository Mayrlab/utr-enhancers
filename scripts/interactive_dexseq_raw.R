...
files <- list.files("data/salmon", "quant.sf", full.names=TRUE, recursive=TRUE) %>%
    { .[str_detect(.,"/(p|s)e/(ery_d|lsk)")] } %>%
    { .[str_detect(., "(_ko_|s3c)", negate=TRUE)] } %>%
...
df_ery_qapa <- read_tsv("data/qapa/ery_all_pau_results.txt",
                        col_types="cccccddcdddd______________________")
...
dxd <- DEXSeqDataSet(countData=round(cts), sampleData=df_samples,
                     design=~library_type + strand + sample + exon + cell_type:exon,
                     featureID=rownames(cts),
                     groupID=str_extract(rownames(cts), "^[^_]+"))

dxd <- estimateSizeFactors(dxd)
dxd <- estimateDispersions(dxd)
dxd <- testForDEU(dxd, reducedModel=~library_type + strand + sample + exon)
...
saveRDS(dxd, "data/dexseq/dxd_hsc_eryd_raw.Rds")

write_tsv(dxr.g, "data/dexseq/dxr_hsc_eryd_raw_gene.tsv.gz")
write_tsv(dxr.t, "data/dexseq/dxr_hsc_eryd_raw_tx.tsv.gz")
write_tsv(dxr.full, "data/dexseq/dxr_hsc_eryd_raw_full.tsv.gz")
...
write_tsv(dex.padj, "data/dexseq/staged_qvals_hsc_eryd_raw_fdr5.tsv.gz")
